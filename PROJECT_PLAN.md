...
## 핵심 플로우
소셜 로그인 → EXIF 사진 선택(자동 시간, 위치는 수동 입력) → Solar 감성 일기/뱃지
...
2. ~~**EXIF 데이터 소실**~~ — **[해결됨, 범위 확대]** 실제 폰 스파이크 결과
   카톡/스크린샷뿐 아니라 **카메라로 직접 찍은 사진도** Android 사진 선택기를
   통하면 GPS EXIF가 강제로 지워짐(시간 정보는 살아있음)을 확인. 그래서
   "fallback"이 아니라 **수동 위치 입력을 기본 경로로** 채택 — 얻을 수 있는
   메타데이터(시간)는 자동으로 채우고, 위치는 사용자가 지도에서 직접 찍음.
   GPS 자동 핀은 커스텀 개발 빌드(EAS Build)가 있어야 시도 가능한 영역이라
   당장은 보류. 상세 내용은 `DECISION_LOG.md` 참조.
