...
## [Phase 3] 지도로 위치 선택 + 날짜 수동 수정

- **시도**: 위 한계 두 가지를 보완. `react-native-maps`(Phase 1 스파이크에서
  Android 호환 이미 확인된 라이브러리, 재검증 없이 바로 사용)로 위치 선택 UI,
  방문 시각을 읽기 전용에서 수정 가능한 텍스트 입력으로 변경.
- **선택**: 별도 라우트로 지도 화면을 옮기지 않고, `places/new.tsx` 안에서
  `showMap` 상태로 폼과 전체화면 지도를 토글하는 방식 채택 — Expo Router에서
  화면 간 값을 되돌려받는 것보다 같은 컴포넌트 안 상태 토글이 훨씬 단순함.
  지도 탭 → 마커 → "확인"으로 좌표를 폼에 반영.
- **결과**: Android Expo Go에서 실제 확인 — 지도 탭으로 위치 선택, 방문 시각
...
## [Phase 4 완료] 앱 내 공유 기능

- **시도**: 트립 상세에 "공유" 버튼 추가, React Native 코어 `Share.share()`로
  제목/뱃지/기간/Solar 에세이를 텍스트로 구성해 OS 공유 시트 호출.
- **선택**: 별도 패키지 없이 RN 코어 API만 사용 — Expo 전용 네이티브 모듈이
  아니라 Expo Go 호환성 문제 자체가 생길 수 없는 API. Solar 요약이 없는
  여행도 공유 가능하게 항목별로 있는 것만 포함.
- **결과**: Android Expo Go에서 실제 확인 — 공유 시트 정상 호출, 텍스트 내용
